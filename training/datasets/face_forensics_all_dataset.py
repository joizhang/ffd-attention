...
def get_face_forensics_all_dataloader(model_cfg, args):
    ...
    for manipulation_type in MANIPULATION_TYPES:
        train_df = pd.read_csv(f'data/{FACE_FORENSICS}/{args.compression_version}/data_{FACE_FORENSICS}_{manipulation_type}_train.csv')
        train_df['fake_type'] = manipulation_type
    ...
    train_data = FaceForensicsDataset(data_root=args.data_dir, df=train_df_concat, mode='train',
                                      ...
                                      compression_version=args.compression_version, use_generalization=False)
    if args.distributed:
    ...
    val_df_list = []
    for manipulation_type in MANIPULATION_TYPES:
        val_df = pd.read_csv(f'data/{FACE_FORENSICS}/{args.compression_version}/data_{FACE_FORENSICS}_{manipulation_type}_val.csv')
        val_df['fake_type'] = manipulation_type
    ...
    val_data = FaceForensicsDataset(data_root=args.data_dir, df=val_df_concat, mode='validation',
                                    transform=val_transform, fake_type='All',
                                    compression_version=args.compression_version)
...
def get_face_forensics_all_test_dataloader(model_cfg, args):
    ...
    test_df_list = []
    for manipulation_type in MANIPULATION_TYPES:
        test_df = pd.read_csv(f'data/{FACE_FORENSICS}/{args.compression_version}/data_{FACE_FORENSICS}_{manipulation_type}_test.csv')
        test_df['fake_type'] = manipulation_type
    ...
    test_data = FaceForensicsDataset(data_root=args.data_dir, df=test_df_concat, mode='test', transform=test_transform,
                                     fake_type='All', compression_version=args.compression_version)
